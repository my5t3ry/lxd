...
func (d *nicBridged) setFilters() (err error) {
	...
	if err == db.ErrNoSuchObject || n == nil {
		if shared.IsTrue(d.config["security.ipv4_filtering"]) && d.config["ipv4.address"]== "" && d.config["nictype"] != "bridged" ||
			...
			return fmt.Errorf("IP filtering requires using a managed parent bridge, or using a unmanaged bridge with accordringly configured address config entries for each filter.")
		}
	}
